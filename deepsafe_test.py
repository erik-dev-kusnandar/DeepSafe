...
class DeepSafeTestOrchestrator:
    def __init__(
        self, media_type: Optional[str], cli_output_dir_base: Optional[str] = None
    ):
        self.media_type = (
            media_type  # Can be None for commands like 'visualize' from file
        )
        self.config_manager = ConfigManager()  # Singleton

        if not self.config_manager.is_config_loaded_successfully():
            console.print(
                "[bold red]FATAL: Configuration could not be loaded. Exiting.[/bold red]"
            )
            sys.exit(1)

        if self.media_type and not self.config_manager.get_media_config(
            self.media_type
        ):
            console.print(
                f"[bold red]Error: Media type '{self.media_type}' not defined in configuration {self.config_manager.config_path}.[/bold red]"
            )
            available_types = list(self.config_manager.get("media_types", {}).keys())
            console.print(
                f"Available media types in config: {available_types if available_types else 'None found'}"
            )
            sys.exit(1)

        self.api_client = APIClient(
            self.config_manager, self.media_type, run_from_host=True
        )
        self.media_handler = MediaHandler(self.config_manager)
        self.results_processor = ResultsProcessor(self.config_manager)

        # Determine base output directory
        _output_dir_base = cli_output_dir_base or self.config_manager.get_default(
            "default_output_dir_base", "./deepsafe_test_results"
        )
        ...
        try:
            ...
            console.print(
                f"[cyan]Results for this run will be saved in: {os.path.abspath(self.current_run_output_dir)}[/cyan]"
            )
        except OSError as e:
            console.print(
                f"[bold red]Error creating output directory {self.current_run_output_dir}: {e}[/bold red]"
            )
            # Fallback to a local temp dir if creation fails, or exit
            self.current_run_output_dir = f"./deepsafe_temp_results_{timestamp}"
            os.makedirs(self.current_run_output_dir, exist_ok=True)
            console.print(
                f"[yellow]Fell back to temporary output directory: {self.current_run_output_dir}[/yellow]"
            )

        self.visualizer = Visualizer(self.config_manager, self.current_run_output_dir)

    def _get_api_client_for_media_type(
        self, media_type_override: Optional[str] = None
    ) -> APIClient:
    ...
    def execute_health_check(self):
        if not self.media_type:
            console.print(
                "[bold red]Health check requires --media-type to specify which models to check.[/bold red]"
            )
            return

        console.print(
            Panel(
                f"[bold cyan]DeepSafe System Health Check (Media Type: {self.media_type.upper()})[/bold cyan]",
                border_style="blue",
                expand=False,
            )
        )
        ...
        main_api_health = client.check_main_api_health(
            force_refresh=True
        )  # force_refresh not used by client, but ok

        api_table = Table(title="DeepSafe System Status")
        api_table.add_column(
            "Component", style="cyan", overflow="fold", min_width=30
        )  # Increased min_width
        api_table.add_column(
            "Status", style="magenta", min_width=25
        )  # Increased min_width
        api_table.add_column(
            "Details", style="green", overflow="fold", max_width=60
        )  # Adjusted max_width

        # Get overall API status
        api_overall_status = main_api_health.get(
            "overall_api_status", "UNKNOWN (Key 'overall_api_status' missing)"
        )
        api_style = (
            "green"
            if api_overall_status.lower() == "healthy"
            else "yellow" if api_overall_status.lower() == "degraded" else "red"
        )
        api_table.add_row(
            "API Gateway Overall",
            f"[bold {api_style}]{api_overall_status.upper()}[/bold {api_style}]",
            main_api_health.get("request_id", ""),
        )

        # Get details for the specific media type
        media_type_details = main_api_health.get("media_type_details", {}).get(
            self.media_type, {}
        )

        media_type_general_status = media_type_details.get(
            "status", "UNKNOWN (Media type details missing or status key missing)"
        )
        media_type_status_style = (
            "green"
            if media_type_general_status.lower() == "healthy"
            else "yellow" if "degraded" in media_type_general_status.lower() else "red"
        )
        api_table.add_row(
            f"'{self.media_type.upper()}' Processing Status",
            f"[bold {media_type_status_style}]{media_type_general_status.upper()}[/bold {media_type_status_style}]",
            "",
        )

        # Get model statuses for the specific media type
        model_statuses_for_this_type = media_type_details.get(
            "models", {}
        )  # CORRECTED ACCESS
        ...
        api_table.add_row(
            RichText(
                f"Individual Models for '{self.media_type.upper()}' (Status via API Gateway)",
                style="bold blue",
            ),
            "",
            "",
        ...
        configured_models_for_type = list(
            self.config_manager.get_model_endpoints(self.media_type).keys()
        )

        if configured_models_for_type:
            if (
                model_statuses_for_this_type
            ):  # Check if we got any model statuses for this type
                for model_name_cfg in configured_models_for_type:
                    model_health_info = model_statuses_for_this_type.get(
                        model_name_cfg,
                        {
                            "status": "NOT_REPORTED_FOR_TYPE",
                            "message": "Not in API health response for this media type.",
                        },
                    )
                    ...
                    status_style = (
                        ...
                        if status.lower() == "healthy"
                            ...
                            in [
                                ...
                                "degraded",
                                "degraded_not_loaded",
                                "degraded_components_not_loaded",
                            ]
                            else "red"
                        )
                    )
                    ...
                    if (
                        "message" in model_health_info and model_health_info["message"]
                    ):  # Check if message is not empty
                        details += f" | {model_health_info['message']}"
                    elif (
                        not details
                    ):  # If no device and no message, provide a placeholder
                        details = "No further details from model health."
                    api_table.add_row(
                        model_name_cfg,
                        f"[bold {status_style}]{status.upper()}[/bold {status_style}]",
                        details.strip(" | "),
                    )
            else:
                api_table.add_row(
                    f"No model statuses reported by API for '{self.media_type}'.",
                    "-",
                    "Check API health endpoint response structure.",
                )
        else:
            api_table.add_row(
                f"No models configured locally for '{self.media_type}'.", "-", "-"
            )

        api_table.add_section()
        stacking_loaded_for_type = media_type_details.get(
            "stacking_ensemble_loaded", False
        )
        ...
        api_table.add_row(
            f"Stacking Ensemble for '{self.media_type.upper()}'",
            f"[bold {stacking_style}]{stacking_status_msg}[/bold {stacking_style}]",
            "",
        )
    ...
    def execute_single_test(
        ...
        if not self.media_type:  # Should be caught by argparse
            console.print("[bold red]Single test requires --media-type.[/bold red]")
            return
        if not self.media_handler.validate_media_file(media_path, self.media_type):
            return

        console.print(
            Panel(
                f"[bold cyan]DeepSafe Single {self.media_type.capitalize()} Test[/bold cyan]\n"
        ...
        if not encoded_media:
            console.print(
                f"[bold red]Failed to encode {media_path}. Aborting test.[/bold red]"
            )
            return
        ...
        if specific_model_name:
            console.print(
                f"Testing with specific model: [magenta]{specific_model_name}[/magenta]"
            )
            model_result = client.test_with_individual_model(
                specific_model_name, media_path, encoded_media, threshold
            )
            ...
            if not final_output_path:
                final_output_path = os.path.join(
                    ...
                    f"test_{Path(media_path).stem}_{specific_model_name}.json",
                )
        else:
            console.print(
                f"Testing with Main API (Ensemble: [magenta]{ensemble_method}[/magenta]) and all individual {self.media_type} models."
            )
            test_results_payload["test_config"][
                "ensemble_method_main_api"
            ] = ensemble_method

            api_result = client.test_with_main_api(
                media_path, self.media_type, encoded_media, threshold, ensemble_method
            )
            test_results_payload["api_ensemble_result"] = api_result
            self._display_api_ensemble_result(api_result)

            individual_model_results = {}
            model_names_for_type = list(
                self.config_manager.get_model_endpoints(self.media_type).keys()
            )
            if not model_names_for_type:
                console.print(
                    f"[yellow]No individual models configured for '{self.media_type}'. Skipping.[/yellow]"
                )
            else:
                with Progress(
                    SpinnerColumn(),
                    ...
                    task = progress.add_task(
                        f"[cyan]Testing individual models...",
                        total=len(model_names_for_type),
                    )
                    for model_name in model_names_for_type:
                        progress.update(
                            task, description=f"[cyan]Testing {model_name}..."
                        )
                        res = client.test_with_individual_model(
                            model_name, media_path, encoded_media, threshold
                        )
                        individual_model_results[model_name] = res
                        progress.advance(task)
                test_results_payload["individual_model_results"] = (
                    individual_model_results
                )
            ...
            if not final_output_path:
                final_output_path = os.path.join(
                    self.current_run_output_dir,
                    f"test_{Path(media_path).stem}_all.json",
                )

        if final_output_path:
            os.makedirs(
                os.path.dirname(os.path.abspath(final_output_path)), exist_ok=True
            )
            with open(final_output_path, "w") as f:
                json.dump(test_results_payload, f, indent=2, cls=NpEncoder)
            console.print(
                f"[green]Test results saved to: {os.path.abspath(final_output_path)}[/green]"
            )

    def _display_single_model_result(
        self, result_dict: Dict[str, Any], model_name: str
    ):
        table = Table(
            title=f"Result for Model: {model_name}",
            show_header=False,
            box=None,
            padding=(0, 1),
        )
        ...
        if "error" in result_dict:
            ...
            cl = result_dict.get("class", "N/A").upper()
            pr = result_dict.get("probability", 0.0)
            it = result_dict.get(
                "inference_time", result_dict.get("total_request_time")
            )
            ts = f"{it:.3f}s" if it else "N/A"
        ...
        console.print(
            Panel(
                table,
                title=f"[white]Model: {model_name}[/white]",
    ...
    def _display_api_ensemble_result(self, api_result: Dict[str, Any]):
        if "error" in api_result:
            console.print(
                Panel(
                    f"[bold red]Error with Main API (Ensemble): {api_result['error']}[/bold red]",
                    border_style="red",
                    expand=False,
                )
            )
        ...
        v = api_result.get("verdict", "N/A").upper()
        c = api_result.get("confidence_in_verdict", 0.0)
        pf = api_result.get("ensemble_score_is_fake", 0.0)
        em = api_result.get("ensemble_method_used", "N/A")
        at = api_result.get("total_inference_time_seconds")
        ct = api_result.get("client_request_time", 0.0)
        col = "red" if v == "FAKE" else "green" if v == "REAL" else "yellow"

        table = Table(
            title=f"Main API Ensemble ({em.capitalize()}) Verdict",
            show_header=False,
            box=None,
            padding=(0, 1),
        )
        ...
        table.add_row(
            "Base Fake Votes:", str(api_result.get("base_model_fake_votes", "N/A"))
        )
        table.add_row(
            "Base Real Votes:", str(api_result.get("base_model_real_votes", "N/A"))
        )
        ...
        console.print(
            Panel(
                table,
                title=f"[white]API Ensemble ({em.capitalize()})[/white]",
                border_style=col,
                expand=False,
            )
        )

    def _display_individual_model_results_table(
        self, model_results: Dict[str, Dict[str, Any]]
    ):
        table = Table(
            title=f"Individual Model Results ({self.media_type.capitalize()})",
            show_lines=True,
        )
        ...
        for name, res in sorted(model_results.items()):
            if "error" in res:
                table.add_row(
                    ...
                    "-",
                    Text(res.get("error", "?"), overflow="ellipsis", max_length=20),
                )
            else:
                cl = res.get("class", "N/A").upper()
                pr = res.get("probability", 0.0)
                it = res.get("inference_time", res.get("total_request_time"))
                ts = f"{it:.3f}" if it else "N/A"
                col = "red" if cl == "FAKE" else "green" if cl == "REAL" else "yellow"
                table.add_row(
                    name,
                    f"[bold {col}]{cl}[/bold {col}]",
                    f"{pr:.3%}" if isinstance(pr, float) else "N/A",
                    ts,
                )
        console.print(table)
    ...
    def execute_batch_processing(
        ...
        if not self.media_type:
            console.print(
                "[bold red]Batch processing requires --media-type.[/bold red]"
            )
            return None

        console.print(
            Panel(
                f"[bold cyan]DeepSafe Batch Processing ({self.media_type.capitalize()})[/bold cyan]\n"
        ...
        models_to_run = (
            [specific_model_name]
            if specific_model_name
            else list(self.config_manager.get_model_endpoints(self.media_type).keys())
        )
        if not models_to_run:
            console.print(
                f"[red]No models configured for {self.media_type}. Aborting batch.[/red]"
            )
            return None
        ...
        with Progress(
            ...
            for model_name in models_to_run:
                task_id = progress.add_task(
                    f"Model [m]{model_name}[/m]", total=len(media_files)
                )
                success_count = 0
                for media_path, gt_label in media_files:
                    ...
                    progress.update(
                        task_id,
                        description=f"Model [m]{model_name}[/m]: Processing {fname}",
                    )
                    encoded = self.media_handler.encode_media_to_base64(media_path)
                    res = (
                        {"error": "Encoding failed"}
                        if not encoded
                        else client.test_with_individual_model(
                            model_name, media_path, encoded, threshold
                        )
                    )
                    res.update(
                        {
                    ...
                    gc.collect()
                console.print(
                    f"Model [m]{model_name}[/m]: {success_count}/{len(media_files)} successful."
                )
                if self.config_manager.get_default("memory_optimization", True):
                    client.request_model_unload(model_name)
                    time.sleep(0.5)
        ...
        for r in all_individual_results:
            p = r.get("media_path")
            if p:
                results_by_media.setdefault(p, []).append(r)
            ...
            for single_media_outputs in results_by_media.values():
                if single_media_outputs:  # Ensure there are results for this path
                    final_results_list.append(
                        self.results_processor.compute_local_ensemble_results(
                            single_media_outputs, threshold, local_ensemble_method
                        )
                    )

        final_out_fpath = output_file_path or os.path.join(
            self.current_run_output_dir,
        ...
        with open(final_out_fpath, "w") as f:
            json.dump(final_results_list, f, indent=2, cls=NpEncoder)
        console.print(
            f"\n[green]Batch raw results saved to: {os.path.abspath(final_out_fpath)}[/green]"
        )

        if display_metrics:
            self._calculate_and_display_metrics(final_results_list, "Batch Processing")
        if generate_plots:
        ...
        console.print(
            f"[green]Batch for {self.media_type} completed. Results & plots in {os.path.abspath(self.current_run_output_dir)}[/green]"
        )
        return {"results": final_results_list, "output_file": final_out_fpath}

    def _calculate_and_display_metrics(
        self, results_list: List[Dict[str, Any]], title_prefix: str
    ):
        ...
        if not metrics_data:
            console.print(
                "[yellow]No metrics could be calculated from the results.[/yellow]"
            )
            return
        ...
        metrics_table = Table(
            title=f"Performance Metrics (Sorted by AUC)", show_lines=True
        )
        metrics_table.add_column(
            "Model/Method", style="cyan", overflow="fold", max_width=35
        )
        ...
        sorted_metric_items = sorted(
            ...
            key=lambda item: (
                item[1].get("auc", -1)
                if isinstance(item[1], dict) and pd.notna(item[1].get("auc"))
                else -1
            ),
            reverse=True,
        )
        ...
        for model_method_name, metrics_dict in sorted_metric_items:
            if isinstance(metrics_dict, dict) and not metrics_dict.get("error"):
                metrics_table.add_row(
                    ...
                    (
                        f"{metrics_dict.get('auc', np.nan):.4f}"
                        if pd.notna(metrics_dict.get("auc"))
                        else "N/A"
                    ),
                ...
                error_msg = (
                    metrics_dict.get("error", "Calc failed")
                    if isinstance(metrics_dict, dict)
                    else "Invalid data"
                )
                count_val = (
                    metrics_dict.get("count", "-")
                    if isinstance(metrics_dict, dict)
                    else "-"
                )
                metrics_table.add_row(
                    model_method_name,
                    str(count_val),
                    Text(error_msg, overflow="ellipsis", max_length=10),
                    "-",
                    "-",
                    "-",
                    "-",
        ...
        metrics_summary_filename = (
            f"{title_prefix.lower().replace(' ', '_')}_{self.media_type}_metrics.json"
        )
        metrics_summary_filepath = os.path.join(
            self.current_run_output_dir, metrics_summary_filename
        )
        with open(metrics_summary_filepath, "w") as f_metrics:
            json.dump(metrics_data, f_metrics, indent=2, cls=NpEncoder)
        console.print(
            f"Metrics summary saved to [green]{os.path.abspath(metrics_summary_filepath)}[/green]"
        )
    ...
    def execute_comparison(
        ...
        if not self.media_type:
            console.print(
                "[bold red]Model comparison requires --media-type.[/bold red]"
            )
            return

        console.print(
            Panel(
                f"[bold cyan]DeepSafe Model Comparison ({self.media_type.capitalize()})[/bold cyan]\n"
                f"Input Dir: {input_dir}\nModel(s): {specific_model_name or 'All for ' + self.media_type}\n"
                f"Threshold: {threshold:.2f}\nLocal Ensemble: {local_ensemble_method.capitalize()}\n"
        ...
        raw_results_filename = (
            f"compare_raw_outputs_{self.media_type}_{Path(input_dir).name}.json"
        )
        raw_results_filepath = os.path.join(
            self.current_run_output_dir, raw_results_filename
        )

        batch_data = self.execute_batch_processing(
            input_dir,
            threshold,
            local_ensemble_method,
            specific_model_name,
            raw_results_filepath,
            display_metrics=True,
            generate_plots=True,  # Ensure metrics and plots are generated by batch
        )
        if batch_data and "results" in batch_data:
            ...
            console.print(
                f"[bold red]Model comparison failed as batch processing encountered an error.[/bold red]"
            )

    def execute_api_stacking_test(self, input_dir: str, threshold: float):
        if not self.media_type:
            console.print(
                "[bold red]API Stacking Test requires --media-type.[/bold red]"
            )
            return

        console.print(
            Panel(
                f"[bold cyan]DeepSafe API Stacking Ensemble Test ({self.media_type.capitalize()})[/bold cyan]\n"
                f"Input Dir: {input_dir}\nAPI Threshold: {threshold:.2f}\n"
                f"Output Dir: {os.path.abspath(self.current_run_output_dir)}\n"
                f"Calls main API with ensemble_method='stacking' for each {self.media_type}.",
                title="API Stacking Test Config",
                border_style="blue",
                expand=False,
            )
        )
        ...
        with Progress(
            ...
            task_id = progress.add_task(
                f"[cyan]Querying API (stacking, {self.media_type})...",
            ...
            for media_path, gt_label in media_files:
                    ...
                    all_api_results.append(
                        {
                            ...
                            "media_name": fname,
                            "ground_truth": gt_label,
                            "model_name": "API_Stacking_Encoding_Error",
                        }
                    )
                    progress.advance(task_id)
                    continue

                api_res = client.test_with_main_api(
                    media_path, self.media_type, encoded, threshold, "stacking"
                )
                ...
                if "error" in api_res:
                    ...
                    for model_key, model_val in api_res.get(
                        "model_results", {}
                    ).items():
        ...
        if not all_api_results:
            console.print("[red]No results from API Stacking test.[/red]")
            return

        raw_out_fpath = os.path.join(
            self.current_run_output_dir,
            f"apistacktest_raw_{self.media_type}_{Path(input_dir).name}.json",
        )
        with open(raw_out_fpath, "w") as f:
            json.dump(all_api_results, f, indent=2, cls=NpEncoder)
        console.print(
            f"\nAPI Stacking Test raw outputs: [green]{os.path.abspath(raw_out_fpath)}[/green]"
        )

        self._calculate_and_display_metrics(
            all_api_results, f"API Stacking Test ({self.media_type.capitalize()})"
        )
    ...
    def execute_benchmark(self, media_path: str, count: int):
        if not self.media_type:
            console.print("[red]Benchmark requires --media-type.[/red]")
            return
        if not self.media_handler.validate_media_file(media_path, self.media_type):
            return
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepSafe Performance Benchmark ({self.media_type.capitalize()})[/bold cyan]\n"
                f"Media File: {media_path}\nIterations: {count}\n"
                f"Output Dir: {os.path.abspath(self.current_run_output_dir)}",
        ...
        with Progress(
            ...
            for _ in range(count):
                try:
                    ...
                    if "client_request_time" in res:
                        results_data["api"]["raw_total_request_times"].append(
                            res["client_request_time"]
                        )
                    time.sleep(0.1)
                except Exception as e:
                    console.print(f"[yellow]API benchmark iter failed: {e}[/yellow]")
                p.advance(task)
        ...
        results_data["api"].update(
            {
                "count": len(api_t),
                "mean_total_request_time": np.mean(api_t) if api_t else None,
        ...
        if models_for_type:
            ...
            for model_name in models_for_type:
                ...
                with Progress(
                    ...
                    task_m = p.add_task(
                        f"Benchmarking [m]{model_name}[/m]", total=count
                    )
                    for _ in range(count):
                        try:
                            res = client.test_with_individual_model(
                                model_name, media_path, encoded_media, def_thresh
                            )
                            if "total_request_time" in res:
                                model_t.append(res["total_request_time"])
                            time.sleep(0.1)
                        except Exception as e:
                            console.print(
                                f"[yellow]Benchmark for {model_name} iter failed: {e}[/yellow]"
                            )
                        p.advance(task_m)
                results_data["models"][model_name] = {
                    "count": len(model_t),
                    "mean_total_request_time": np.mean(model_t) if model_t else None,
                    "median_total_request_time": (
                        np.median(model_t) if model_t else None
                    ),
        ...
        bench_json_path = os.path.join(
            self.current_run_output_dir, f"benchmark_results_{self.media_type}.json"
        )
        with open(bench_json_path, "w") as f:
            json.dump(results_data, f, indent=2, cls=NpEncoder)
        console.print(
            f"\nBenchmark results JSON: [green]{os.path.abspath(bench_json_path)}[/green]"
        )
        self.visualizer.plot_benchmark_summary(
            results_data
        )  # Visualizer handles subdirs
        console.print(
    ...
    def _display_benchmark_table(self, benchmark_results: Dict[str, Any]):
        ...
        table = Table(
            title=f"Benchmark Results (Total Request Time, {self.media_type.capitalize()})",
            show_lines=True,
        )
        ...
        if api_data.get("count", 0) > 0:
            table.add_row(
                "API (Ensemble)",
                str(api_data["count"]),
                f"{api_data.get('mean_total_request_time', np.nan):.4f}",
                f"{api_data.get('median_total_request_time', np.nan):.4f}",
                f"{api_data.get('min_total_request_time', np.nan):.4f}",
                f"{api_data.get('max_total_request_time', np.nan):.4f}",
        ...
        for model_name, data in sorted(models_data.items()):
            if data.get("count", 0) > 0:
                table.add_row(
                    model_name,
                    str(data["count"]),
                    f"{data.get('mean_total_request_time', np.nan):.4f}",
                    f"{data.get('median_total_request_time', np.nan):.4f}",
                    f"{data.get('min_total_request_time', np.nan):.4f}",
                    f"{data.get('max_total_request_time', np.nan):.4f}",
    ...
    def execute_visualization(self, input_path: str, plot_type: str):
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepSafe Visualization Generation[/bold cyan]\n"
            ...
            try:
                ...
                if isinstance(data, list):
                    ...
                    if "results" in data and isinstance(data["results"], list):
                        ...
                        if data["api_ensemble_result"]:
                            results_list.append(data["api_ensemble_result"])
                        results_list.extend(
                            list(data.get("individual_model_results", {}).values())
                        )
                    elif (
                        "specified_model_result" in data
                        and data["specified_model_result"]
                    ):
                        results_list.append(data["specified_model_result"])
                    else:
                        console.print(
                            f"[red]Unrecognized JSON structure in {input_path}.[/red]"
                        )
                        return
                else:
                    console.print(
                        f"[red]Input file {input_path} not a list or recognized dict.[/red]"
                    )
                    return
                console.print(
                    f"Loaded {len(results_list)} results from [cyan]{input_path}[/cyan]."
                )
            except Exception as e:
                console.print(f"[red]Error loading from {input_path}: {e}[/red]")
                return
        elif os.path.isdir(input_path):
            if not self.media_type:
                console.print("[red]'visualize' from dir needs --media-type.[/red]")
                return
            console.print(
                f"[cyan]Input is dir. Processing {self.media_type} files in '{input_path}'...[/cyan]"
            )
            temp_fpath = os.path.join(
                self.current_run_output_dir,
                f"temp_viz_results_{self.media_type}_{Path(input_path).name}.json",
            )
            batch_data = self.execute_batch_processing(
                input_path,
                self.config_manager.get_default("default_threshold", 0.5),
                self.config_manager.get_default("default_ensemble_method", "voting"),
                output_file_path=temp_fpath,
                display_metrics=False,
                generate_plots=False,  # No metrics/plots from this sub-run
            )
            if batch_data and "results" in batch_data:
                results_list = batch_data["results"]
            else:
                console.print(
                    f"[red]Failed to process dir '{input_path}' for visualization.[/red]"
                )
                return
        else:
            console.print(
                f"[red]Input path '{input_path}' not a valid file or directory.[/red]"
            )
            return
        ...
        valid_results = [
            r for r in results_list if isinstance(r, dict) and "error" not in r
        ]
        if not valid_results:
            console.print(
                "[yellow]No valid (non-error) results for visualizations.[/yellow]"
            )
            return

        if plot_type in ["all", "confusion"]:
            self.visualizer.plot_confusion_matrices(valid_results)
        if plot_type in ["all", "roc"]:
            self.visualizer.plot_roc_curves(valid_results)
        if plot_type in ["all", "probability"]:
            self.visualizer.plot_probability_distributions(valid_results)
...
def main_cli():
    ...
    media_type_parser_parent.add_argument(
        "--media-type",
        type=str,
    ...
    media_type_parser_optional_parent.add_argument(
        "--media-type",
        type=str,
        choices=["image", "video", "audio"],
    ...
    health_p = subparsers.add_parser(
        "health", help="Check system health.", parents=[media_type_parser_parent]
    )

    test_p = subparsers.add_parser(
        "test", help="Test a single media file.", parents=[media_type_parser_parent]
    )
    test_p.add_argument(
        "--input",
        type=str,
    ...
    test_p.add_argument(
        "--threshold", type=float, help="Classification threshold (0.0-1.0)."
    )
    test_p.add_argument(
        "--model",
        type=str,
        dest="specific_model_name",
    ...
    test_p.add_argument(
        "--output-file",
        type=str,
        help="Specific output JSON file for single test results.",
    )

    batch_p = subparsers.add_parser(
        "batch",
        help="Process a batch of media files.",
        parents=[media_type_parser_parent],
    )
    batch_p.add_argument(
        "--input-dir",
        type=str,
        required=True,
        help="Input directory (Real/Fake subdirs ideal).",
    )
    batch_p.add_argument(
        "--model",
        type=str,
        dest="specific_model_name",
        help="Specific model to run, or all if omitted.",
    )
    ...
    batch_p.add_argument(
        "--output-file", type=str, help="Specific output JSON file for batch results."
    )

    compare_p = subparsers.add_parser(
        "compare",
        help="Compare model performance on a batch.",
        parents=[media_type_parser_parent],
    )
    compare_p.add_argument(
        ...
        type=str,
        required=True,
        help="Input directory (Real/Fake subdirs).",
    )
    compare_p.add_argument(
        "--model",
        type=str,
        dest="specific_model_name",
        help="Specific model, or all if omitted.",
    )
    ...
    apistack_p = subparsers.add_parser(
        "apistacktest",
        help="Test API's stacking ensemble on a batch.",
        parents=[media_type_parser_parent],
    )
    apistack_p.add_argument(
        "--input-dir",
        type=str,
        required=True,
        help="Input directory (Real/Fake subdirs).",
    ...
    apistack_p.add_argument(
        "--threshold", type=float, help="Threshold for API's stacking decision."
    )

    benchmark_p = subparsers.add_parser(
        "benchmark",
        help="Benchmark API and model latency.",
        parents=[media_type_parser_parent],
    )
    benchmark_p.add_argument(
        ...
        required=True,
        dest="media_path",
        help="Path to a media file for benchmark.",
    )
    benchmark_p.add_argument(
    ...
    visualize_p = subparsers.add_parser(
        ...
        parents=[media_type_parser_optional_parent],
    )
    visualize_p.add_argument(
        "--input-path",
        type=str,
        required=True,
        help="Path to results JSON file OR a directory of media.",
    )
    visualize_p.add_argument(
        "--plot-type",
        type=str,
        default="all",
        choices=["all", "confusion", "roc", "probability"],
    ...
    if hasattr(args, "ensemble_method") and args.ensemble_method is None:
        args.ensemble_method = config_manager.get_default(
            "default_ensemble_method", "stacking"
        )
    if hasattr(args, "local_ensemble_method") and args.local_ensemble_method is None:
        args.local_ensemble_method = config_manager.get_default(
            "default_local_ensemble_method", "voting"
        )  # Add to config if needed

    # Media type logic for orchestrator initialization
    orchestrator_media_type = (
        args.media_type if hasattr(args, "media_type") and args.media_type else None
    )
    if (
        args.command == "visualize"
        and os.path.isdir(args.input_path)
        and not orchestrator_media_type
    ):
        parser.error(
            "Command 'visualize' requires --media-type when --input-path is a directory."
        )

    orchestrator = DeepSafeTestOrchestrator(
        orchestrator_media_type, cli_output_dir_base=args.output_dir
    )
    ...
    try:
        if args.command == "health":
            ...
            if (
                args.specific_model_name
                and args.specific_model_name
                not in config_manager.get_all_model_names(args.media_type)
            ):
                console.print(
                    f"[bold red]Error: Model '{args.specific_model_name}' not in config for '{args.media_type}'.[/bold red]\nAvailable: {config_manager.get_all_model_names(args.media_type)}"
                )
                sys.exit(1)
            orchestrator.execute_single_test(
                args.media_path,
                args.threshold,
            ...
            if (
                ...
                sys.exit(1)
            orchestrator.execute_batch_processing(
                args.input_dir,
                args.threshold,
                args.local_ensemble_method,
                args.specific_model_name,
                args.output_file,
            )
            ...
            if (
                ...
                sys.exit(1)
            orchestrator.execute_comparison(
                args.input_dir,
                args.threshold,
                args.local_ensemble_method,
                args.specific_model_name,
...
if __name__ == "__main__":
    if not config_manager.is_config_loaded_successfully():
        console.print(
            "[bold red]Exiting due to configuration load failure. Please check `config/deepsafe_config.json`.[/bold red]"
        )
        sys.exit(1)
    try:
        main_cli()
