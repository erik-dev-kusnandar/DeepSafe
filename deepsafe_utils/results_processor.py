...
class ResultsProcessor:
    ...
    def compute_local_ensemble_results(
        self,
        ...
        valid_outputs = [
            r
            for r in model_results_for_media_item
        ...
        return {
            ...
            "inference_time": sum(
                r.get("inference_time", r.get("total_request_time", 0)) or 0
                for r in valid_outputs
            ),
        }
    ...
    def calculate_batch_metrics(
        ...
        for res_item in all_results:
            ...
            if not model_name or "error" in res_item:
                continue
            if model_name not in results_grouped:
        ...
        for model_name, model_specific_results in results_grouped.items():
            ...
            for result in model_specific_results:
                if (
                    ...
                    or result.get("probability") is None
                ):
                    continue
            ...
            if not y_true:
                current_metrics["error"] = "No valid ground truth labels found."
            elif len(set(y_true)) < 2 and len(y_true) > 0:
                current_metrics["error"] = "Only one class present in ground truth."
            ...
            try:
                ...
                if not (
                    len(np.unique(y_pred_proba)) < 2
                    and len(y_pred_proba) == len(y_true)
                ):
                    try:
                        current_metrics["auc"] = auc(
                            *roc_curve(y_true, y_pred_proba)[:2]
                        )
                    except ValueError:
                        current_metrics["auc"] = np.nan  # Keep NaN if ROC fails

                cm = confusion_matrix(
                    y_true, y_pred_class, labels=[0, 1]
                ...
                console.print(
                    f"[bold red]Error calculating metrics for {model_name}: {e}[/bold red]"
                )
