...
def load_model_internal():
    ...
    with model_lock:
        ...
        logger.info(
            f"Loading {MODEL_NAME} model from {MODEL_FULL_PATH} onto {DEVICE}..."
        )
        ...
        try:
            ...
            FileNotFoundError
        ) as e_fnf:  # Should be caught by earlier check, but good to have
            logger.error(f"Model file not found during load: {e_fnf}", exc_info=True)
            model = None
            raise RuntimeError(
                f"Model file not found for {MODEL_NAME}: {e_fnf}"
            ) from e_fnf
        except Exception as e_load:  # Catch other torch.load or model init errors
            logger.error(
                f"An unexpected error occurred loading {MODEL_NAME} model: {e_load}",
                exc_info=True,
            )
            model = None
            raise RuntimeError(
                f"Model loading error for {MODEL_NAME}: {e_load}"
            ) from e_load
        finally:
            if DEVICE.type == "cuda":
...
def unload_model_if_idle():
    ...
    with model_lock:
        if model is not None and (time.time() - last_used_time > MODEL_TIMEOUT):
            logger.info(
                f"Unloading {MODEL_NAME} model due to inactivity (timeout: {MODEL_TIMEOUT}s)."
            )
            del model
            model = None
            if DEVICE.type == "cuda":
...
async def startup_event_handler():
    if PRELOAD_MODEL:
        ...
        try:
            ...
            logger.error(
                f"Fatal error during {MODEL_NAME} model preloading: {e}. Service might not function correctly.",
                exc_info=True,
            )
    else:
        logger.info(
            f"{MODEL_NAME} model will be loaded on first request (PRELOAD_MODEL=false)."
        )
    ...
    if not PRELOAD_MODEL and MODEL_TIMEOUT > 0:
        ...
        def periodic_unload_check_runner():
            ...
            if (
                model is not None or PRELOAD_MODEL
            ):  # Keep timer running if model still loaded or preloading
                active_timer_npr = threading.Timer(
                    MODEL_TIMEOUT / 2.0, periodic_unload_check_runner
                )
                active_timer_npr.daemon = (
                    True  # Allow main program to exit even if timer is active
                )
                active_timer_npr.start()
            else:
                logger.info(
                    f"[{MODEL_NAME}] Model unloaded, stopping idle check timer."
                )
        ...
        active_timer_npr.start()
        logger.info(
            f"{MODEL_NAME} model idle check timer initiated (interval: {MODEL_TIMEOUT / 2.0}s)."
        )
...
async def health_check_endpoint():
    ...
    return {
        ...
        "model_definition_imported": "resnet50" in globals()
        and callable(globals()["resnet50"]),
...
async def unload_model_endpoint():
    ...
    if model is None:
        return {
            "status": "not_loaded",
            "message": f"{MODEL_NAME} model is not currently loaded.",
        }
    with model_lock:
        if model is not None:
            ...
            return {
                "status": "unloaded",
                "message": f"{MODEL_NAME} model unloaded successfully.",
            }
        else:
            return {
                "status": "already_unloaded",
                "message": f"{MODEL_NAME} model was already unloaded by another request.",
            }
...
async def predict_image_endpoint(input_data: ImageInput):
    try:
        ...
        if model is None:
            logger.error(
                f"{MODEL_NAME} model is not available for prediction (ensure_model_loaded failed)."
            )
            raise HTTPException(
        ...
        logger.info(
            f"Prediction for {MODEL_NAME} completed in {inference_time:.4f}s. Prob Fake: {probability:.4f}, Threshold: {input_data.threshold}, Class: {class_label}"
        )
        ...
        RuntimeError
    ) as e_rt_pred:  # Catch errors from load_model_internal or PyTorch
        ...
        if (
            "local variable 'resnet50' referenced before assignment" in str(e_rt_pred)
            or "Model loading error" in str(e_rt_pred)
            or "Model file not found" in str(e_rt_pred)
        ):
            # This indicates a problem during model loading, likely due to import or file issues
            raise HTTPException(
                status_code=503, detail=f"Model failed to load: {e_rt_pred}"
            )
        ...
        raise HTTPException(
            status_code=400, detail=f"Image processing error: {e_val_pred}"
        )
    except HTTPException:
        raise
    except Exception as e_pred:
        logger.error(
            f"Unexpected error processing prediction for {MODEL_NAME}: {e_pred}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail=f"Unexpected prediction error: {e_pred}"
        )
