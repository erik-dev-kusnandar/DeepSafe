...
if not torch.cuda.is_available():
    ...
    if not logger_pre_patch_ucf.hasHandlers():
        ...
        formatter_ucf = logging.Formatter(
    ...
    logger_pre_patch_ucf.info(
        "Patching torch.cuda functions for CPU-only environment (ucf_deepfake_detection)."
    )

    def _patched_cuda_is_available_ucf():
        return False

    def _patched_cuda_get_device_name_ucf(device=None):
        return "CPU (Patched by ucf_app)"

    torch.cuda.is_available = _patched_cuda_is_available_ucf
    if hasattr(torch.cuda, "get_device_name"):
        torch.cuda.get_device_name = _patched_cuda_get_device_name_ucf

    if hasattr(torch.cuda, "_get_device_properties"):

        class DummyDevicePropertiesUcf:
            def __init__(self):
                self.name = "CPU (Patched Properties by ucf_app)"

        def _patched_cuda_get_device_properties_ucf(device=None):
            return DummyDevicePropertiesUcf()
...
if deepfake_bench_root_path not in sys.path:
    sys.path.insert(0, deepfake_bench_root_path)
if deepfake_bench_training_path not in sys.path:
    sys.path.insert(0, deepfake_bench_training_path)
...
try:
    ...
    logger.info(
        "Successfully imported DETECTOR, BACKBONE, LOSSFUNC from DeepfakeBench for UCF."
    )
...
XCEPTION_PRETRAINED_PATH_UCF = os.path.join(
    deepfake_bench_training_path, "pretrained/xception-b5690688.pth"
)
DLIB_PREDICTOR_FILE_PATH_FOR_CHECK_UCF = (
    "/app/preprocessing/dlib_tools/shape_predictor_81_face_landmarks.dat"
)
    ...
    if model_config_loaded_ucf is None:
        ...
        if not os.path.exists(UCF_CONFIG_PATH):
            raise FileNotFoundError(
                f"UCF detector config file not found: {UCF_CONFIG_PATH}"
            )

        with open(UCF_CONFIG_PATH, "r") as f:
            detector_specific_config = yaml.safe_load(f)

        general_train_config_path = os.path.join(
            deepfake_bench_training_path, "config/train_config.yaml"
        )
        if os.path.exists(general_train_config_path):
            with open(general_train_config_path, "r") as f_train:
                general_config_defaults = yaml.safe_load(f_train)
            merged_config = {**general_config_defaults, **detector_specific_config}
        else:
            logger.warning(
                f"General train_config.yaml not found. Using only UCF-specific config."
            )
        ...
        if "backbone_config" not in merged_config:
            merged_config["backbone_config"] = {}
        merged_config["backbone_config"]["inc"] = merged_config["backbone_config"].get(
            "inc", 3
        )
        merged_config["backbone_config"]["num_classes"] = merged_config[
            "backbone_config"
        ].get("num_classes", 2)
        merged_config["backbone_config"]["dropout"] = merged_config[
            "backbone_config"
        ].get("dropout", False)
        merged_config["backbone_config"]["mode"] = merged_config["backbone_config"].get(
            "mode", "adjust_channel"
        )
...
def build_ucf_inference_preprocessor():
    ...
    transform_pil = T.Compose(
        [
            T.Resize(
                (resolution, resolution), interpolation=T.InterpolationMode.BILINEAR
            ),
            T.ToTensor(),
...
def load_model_internal_ucf():
    ...
    with model_lock_ucf:
        ...
        logger.info(
            f"[{MODEL_NAME_DISPLAY}] Initializing UCF model (via DeepfakeBench registry)..."
        )
        try:
            config = load_ucf_config_from_yaml()
            config["cuda"] = False
            ...
            if not os.path.exists(DLIB_PREDICTOR_FILE_PATH_FOR_CHECK_UCF):
                logger.error(
                    f"Dlib shape predictor NOT FOUND at: {DLIB_PREDICTOR_FILE_PATH_FOR_CHECK_UCF}"
                )
            else:
                logger.info(
                    f"Dlib shape predictor found at: {DLIB_PREDICTOR_FILE_PATH_FOR_CHECK_UCF}"
                )

            if UCF_DETECTOR_KEY_IN_REGISTRY not in DETECTOR.data:
                raise RuntimeError(
                    f"UCF detector ('{UCF_DETECTOR_KEY_IN_REGISTRY}') not registered."
                )
            ...
            if not os.path.exists(UCF_WEIGHTS_PATH):
                raise FileNotFoundError(
                    f"UCF model weights not found: {UCF_WEIGHTS_PATH}"
                )
            ...
            last_used_time_ucf = time.time()
            logger.info(
                f"[{MODEL_NAME_DISPLAY}] UCF Model and preprocessor loaded successfully on CPU."
            )
...
def unload_model_if_idle_ucf():
    ...
    if model_instance_ucf is None or PRELOAD_MODEL:
        return
    with model_lock_ucf:
        if model_instance_ucf is not None and (
            time.time() - last_used_time_ucf > MODEL_TIMEOUT
        ):
            logger.info(f"[{MODEL_NAME_DISPLAY}] Unloading model due to inactivity.")
            del model_instance_ucf
            del ucf_image_preprocessor_fn
            model_instance_ucf, ucf_image_preprocessor_fn = None, None
            gc.collect()
            logger.info(f"[{MODEL_NAME_DISPLAY}] Model unloaded.")
...
async def startup_event_handler_ucf():
    if PRELOAD_MODEL:
        ...
        try:
            ...
            logger.error(f"[{MODEL_NAME_DISPLAY}] Preloading error: {e}", exc_info=True)
    else:
    ...
    if not PRELOAD_MODEL and MODEL_TIMEOUT > 0:
        ...
        def periodic_unload_check_runner_ucf():
            ...
            if model_instance_ucf is not None or PRELOAD_MODEL:
                active_timer_ucf = threading.Timer(
                    MODEL_TIMEOUT / 2.0, periodic_unload_check_runner_ucf
                )
...
async def get_root_ucf():
    return {
        ...
        "model_loaded_status": model_instance_ucf is not None
        and ucf_image_preprocessor_fn is not None,
    }
...
async def get_health_ucf():
    model_files_ok = (
        os.path.exists(UCF_WEIGHTS_PATH)
        and os.path.exists(UCF_CONFIG_PATH)
        and os.path.exists(XCEPTION_PRETRAINED_PATH_UCF)
        and os.path.exists(DLIB_PREDICTOR_FILE_PATH_FOR_CHECK_UCF)
    )

    model_and_preprocessor_loaded = (
        model_instance_ucf is not None and ucf_image_preprocessor_fn is not None
    )
    ...
    if not model_files_ok:
        ...
        missing_items = []
        if not os.path.exists(UCF_WEIGHTS_PATH):
            missing_items.append("UCF weights")
        if not os.path.exists(UCF_CONFIG_PATH):
            missing_items.append("UCF config")
        if not os.path.exists(XCEPTION_PRETRAINED_PATH_UCF):
            missing_items.append("Xception backbone weights")
        if not os.path.exists(DLIB_PREDICTOR_FILE_PATH_FOR_CHECK_UCF):
            missing_items.append(f"Dlib predictor")
    ...
    return {
        ...
        "details": {
            ...
            "xception_backbone_weights_found": os.path.exists(
                XCEPTION_PRETRAINED_PATH_UCF
            ),
            "dlib_shape_predictor_found": os.path.exists(
                DLIB_PREDICTOR_FILE_PATH_FOR_CHECK_UCF
            ),
...
async def post_predict_ucf(input_data: ImageInput) -> Dict[str, Any]:
    try:
        ...
        if model_instance_ucf is None or ucf_image_preprocessor_fn is None:
            raise HTTPException(
                status_code=503, detail="UCF Model/Preprocessor not available."
            )
        ...
        image_tensor_rgb_norm = (
            ucf_image_preprocessor_fn(pil_image).unsqueeze(0).to(DEVICE)
        )
        ...
        with torch.no_grad():
            prediction_output_dict = model_instance_ucf.forward(
                data_dict_for_model_forward, inference=True
            )

        # --- Check if 'prob' key exists, if not, calculate from 'cls' ---
        if (
            "prob" in prediction_output_dict
            and prediction_output_dict["prob"] is not None
        ):
            probability_fake = prediction_output_dict["prob"].item()
        elif (
            "cls" in prediction_output_dict
            and prediction_output_dict["cls"] is not None
        ):
            logger.warning(
                f"[{MODEL_NAME_DISPLAY}] 'prob' key not found in UCFDetector output, calculating from 'cls' (logits)."
            )
            logits = prediction_output_dict[
                "cls"
            ]  # Assuming 'cls' contains logits [batch_size, num_classes]
            probabilities = F.softmax(logits, dim=1)
            probability_fake = probabilities[
                0, 1
            ].item()  # Probability of being FAKE (class 1)
        else:
            logger.error(
                f"[{MODEL_NAME_DISPLAY}] Neither 'prob' nor 'cls' key found in UCFDetector output for probability calculation."
            )
        ...
        logger.info(
            f"[{MODEL_NAME_DISPLAY}] Prediction: P(Fake)={probability_fake:.4f}, Time={inference_time_seconds:.4f}s"
        )
        ...
        logger.error(
            f"[{MODEL_NAME_DISPLAY}] File not found error during UCF prediction: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=503, detail=f"Model resources missing for UCF: {e}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error during UCF prediction: {e}"
        )
...
async def post_unload_ucf():
    global model_instance_ucf, ucf_image_preprocessor_fn
    if model_instance_ucf is None:
        return {"status": "not_loaded", "message": "UCF model not loaded."}
    with model_lock_ucf:
        if model_instance_ucf is not None:
            del model_instance_ucf
            del ucf_image_preprocessor_fn
            model_instance_ucf, ucf_image_preprocessor_fn = None, None
            gc.collect()
            logger.info(f"[{MODEL_NAME_DISPLAY}] UCF Model manually unloaded.")
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Starting {MODEL_NAME_DISPLAY} server on port {port} (CPU, CUDA patched)"
    )
