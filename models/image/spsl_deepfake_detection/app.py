...
if not torch.cuda.is_available():
    ...
    logger_pre_patch.propagate = (
        False  # Prevent duplicate handlers if root logger also gets configured
    )
    ...
    logger_pre_patch.info(
        "Patching torch.cuda functions for CPU-only environment (spsl_deepfake_detection)."
    )
...
try:
    ...
    logger.info(
        "Successfully imported DETECTOR, BACKBONE, LOSSFUNC from DeepfakeBench."
    )
...
XCEPTION_PRETRAINED_PATH = os.path.join(
    deepfake_bench_training_path, "pretrained/xception-b5690688.pth"
)
DLIB_PREDICTOR_FILE_PATH_FOR_CHECK = (
...
last_used_time = 0.0
spsl_rgb_preprocessor_fn: Optional[callable] = (
    None  # Renamed to clarify it's for RGB only
)
...
class ImageInput(BaseModel):
    ...
    threshold: Optional[float] = Field(
        0.7, ge=0.0, le=1.0, description="Classification threshold"
    )
...
def load_spsl_config_from_yaml():
    ...
    if model_config_loaded is None:
        ...
        if not os.path.exists(SPSL_CONFIG_PATH):
            ...
            if os.path.exists(config_dir_to_check):
                logger.error(
                    f"Contents of {config_dir_to_check}: {os.listdir(config_dir_to_check)}"
                )
            else:
                logger.error(f"Config directory {config_dir_to_check} does not exist.")
            raise FileNotFoundError(
                f"SPSL detector config file not found: {SPSL_CONFIG_PATH}"
            )

        with open(SPSL_CONFIG_PATH, "r") as f:
        ...
        if os.path.exists(general_train_config_path):
            ...
            logger.warning(
                f"General train_config.yaml not found. Using only detector-specific config."
            )
        ...
        merged_config["backbone_config"][
            "inc"
        ] = 4  # SPSL's Xception expects 4 input channels internally
...
def load_model_internal():
    ...
    with model_lock:
        ...
        logger.info(
            f"[{MODEL_NAME_DISPLAY}] Initializing SPSL model (via DeepfakeBench registry)..."
        )
        try:
            config = load_spsl_config_from_yaml()
            config["cuda"] = False
            ...
            if not os.path.exists(DLIB_PREDICTOR_FILE_PATH_FOR_CHECK):
                logger.error(
                    f"Dlib shape predictor NOT FOUND at: {DLIB_PREDICTOR_FILE_PATH_FOR_CHECK} (relative to CWD /app)"
                )
            else:
                logger.info(
                    f"Dlib shape predictor found at: {DLIB_PREDICTOR_FILE_PATH_FOR_CHECK}"
                )

            if SPSL_DETECTOR_KEY_IN_REGISTRY not in DETECTOR.data:
                raise RuntimeError(
                    f"SPSL detector ('{SPSL_DETECTOR_KEY_IN_REGISTRY}') not registered."
                )
            ...
            if not os.path.exists(SPSL_WEIGHTS_PATH):
                raise FileNotFoundError(
                    f"SPSL model weights not found: {SPSL_WEIGHTS_PATH}"
                )
            ...
            spsl_rgb_preprocessor_fn = (
                build_spsl_rgb_image_preprocessor()
            )  # Correct preprocessor
            last_used_time = time.time()
            logger.info(
                f"[{MODEL_NAME_DISPLAY}] SPSL Model and RGB preprocessor loaded successfully on CPU."
            )
    ...
    if not PRELOAD_MODEL and MODEL_TIMEOUT > 0:
        ...
        def periodic_unload_check_runner_spsl():
            ...
            if model_instance is not None or PRELOAD_MODEL:
                active_timer_spsl = threading.Timer(
                    MODEL_TIMEOUT / 2.0, periodic_unload_check_runner_spsl
                )
...
async def get_root_spsl():
    return {
        ...
        "model_loaded_status": model_instance is not None
        and spsl_rgb_preprocessor_fn is not None,
    }
...
async def get_health_spsl():
    model_files_ok = (
        os.path.exists(SPSL_WEIGHTS_PATH)
        and os.path.exists(SPSL_CONFIG_PATH)
        and os.path.exists(XCEPTION_PRETRAINED_PATH)
        and os.path.exists(DLIB_PREDICTOR_FILE_PATH_FOR_CHECK)
    )

    model_and_preprocessor_loaded = (
        model_instance is not None and spsl_rgb_preprocessor_fn is not None
    )
    ...
    if not model_files_ok:
        ...
        missing_items = []
        if not os.path.exists(SPSL_WEIGHTS_PATH):
            missing_items.append("SPSL weights")
        if not os.path.exists(SPSL_CONFIG_PATH):
            missing_items.append("SPSL config")
        if not os.path.exists(XCEPTION_PRETRAINED_PATH):
            missing_items.append("Xception backbone weights")
        if not os.path.exists(DLIB_PREDICTOR_FILE_PATH_FOR_CHECK):
            missing_items.append(
                f"Dlib predictor at {DLIB_PREDICTOR_FILE_PATH_FOR_CHECK}"
            )
    ...
    return {
        ...
        "details": {
            ...
            "dlib_shape_predictor_found (at /app/preprocessing/...)": os.path.exists(
                DLIB_PREDICTOR_FILE_PATH_FOR_CHECK
            ),
...
async def post_predict_spsl(input_data: ImageInput) -> Dict[str, Any]:
    try:
        ...
        if model_instance is None or spsl_rgb_preprocessor_fn is None:
            raise HTTPException(
                status_code=503, detail="SPSL Model/Preprocessor not available."
            )
        ...
        image_tensor_rgb_norm = (
            spsl_rgb_preprocessor_fn(pil_image).unsqueeze(0).to(DEVICE)
        )
        ...
        with torch.no_grad():
            prediction_output_dict = model_instance.forward(
        ...
        logger.error(
            f"[{MODEL_NAME_DISPLAY}] File not found during prediction: {e}",
            exc_info=True,
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error during SPSL prediction: {e}"
        )
...
async def post_unload_spsl():
    ...
    if model_instance is None:
        return {"status": "not_loaded", "message": "SPSL model not loaded."}
    with model_lock:
        if model_instance is not None:
            del model_instance
            del spsl_rgb_preprocessor_fn
            model_instance, spsl_rgb_preprocessor_fn = None, None
            gc.collect()
            logger.info(f"[{MODEL_NAME_DISPLAY}] SPSL Model manually unloaded.")
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Starting {MODEL_NAME_DISPLAY} server directly on port {port} (CPU explicitly, CUDA patched)"
    )
